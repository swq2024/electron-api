...
const sendErr = (res, error) => {
    ...
    // 2. Sequelize 数据库错误处理
    else if (error.name === 'SequelizeValidationError') { // 数据验证错误
        statusCode = 400;
        responseMessage = '数据验证失败';
        errors = error.errors.map(val => ({ field: val.path, message: val.message })); // 保留字段详情
    } else if (error.name === 'SequelizeUniqueConstraintError') { // 唯一性约束错误
        statusCode = 409;
        const field = error.errors[0].path;
        responseMessage = `${field.charAt(0).toUpperCase() + field.slice(1)} 已存在`;
        errors = [responseMessage];
    } else if (error.name === 'SequelizeForeignKeyConstraintError') { // 无效的关联 ID（外键约束失败）
        statusCode = 400;
        responseMessage = '无效的关联 ID（外键约束失败）';
        errors = [responseMessage];
    } else if (error.name === 'SequelizeOptimisticLockError') { // 乐观锁冲突错误
        statusCode = 409;
    ...
    // 3. JWT 认证错误处理
    else if (error.name === 'JsonWebTokenError') { // JWT 验证失败（例如，无效的令牌）
        statusCode = 401;
        responseMessage = 'Token 无效，请重新登录';
        errors = [responseMessage];
    } else if (error.name === 'TokenExpiredError') { // JWT 过期错误
        statusCode = 401;
    ...
    // 4. 文件上传错误处理
    else if (error instanceof multer.MulterError) { // 文件上传错误，例如文件大小限制或格式问题
    ...
    else if (error instanceof createError.HttpError) { // 例如，404 Not Found, 403 Forbidden 等错误
